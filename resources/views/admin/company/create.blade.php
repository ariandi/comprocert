...
<ol class="breadcrumb">
    ...
    <li class="active"><a href="{{ url('admin/companies/create') }}">@yield('title')</a></li>
</ol>
@endsection
		...
		{{-- @if(isset($company->id))
			<div class="row" style="margin-bottom: 15px;">
				<div class="col-md-4 text-center" style="margin-bottom: 15px;">
					...
					target="{{ route('product.editajax', ['id' => $company->id]) }}">Product Detail</button>
				</div>

				<div class="col-md-4 text-center" style="margin-bottom: 15px;">
					<button class="btn btn-success btn-block productTextAjax" type="button" thirdparty="tinymce"
					target="{{ route('product.editproducttext', ['id' => $company->id]) }}">Product Text</button>
				</div>

				<div class="col-md-4 text-center" style="margin-bottom: 15px;">
					<button class="btn btn-success btn-block productTextAjax" type="button" 
					target="{{ route('product.editproductmedia', ['id' => $company->id]) }}">Product Images</button>
				</div>
			</div>
		@endif --}}


		<form action="{{ route('companies.store') }}" method="post" enctype="multipart/form-data" id="upload_form">
		{{ csrf_field() }}
		@if(isset($company->id))
	              	...
	              	<div class="row">
	              		<div class="col-md-6">
			                ...
			                <div class="form-group">
			                  ...
			                  <div class="col-sm-9">
			                    <b>{{ $company->InsertedByPersonID }}</b>
			                  </div>
			                </div>
...
@section('addingScriptJs')
	...
	$(document).ready(function() {
	    ...
	    $('body').on('click', '.saveProductText', function(){
	      ...
	      $.ajax({
	        method:"POST",
	        url:"{{ route('product.updateajax', ['id' => $company->id]) }}",
	        data: formData,
	        //dataType: "json",
	        success: function(result){
	            console.log(result);
	    ...
	    $("body").on('click', '.saveProductMedia', function(){
	    	...
	    	$.ajax({
	        method:"POST",
	        url:"{{ route('product.updateproductmediaajax', ['id' => $company->id]) }}",
